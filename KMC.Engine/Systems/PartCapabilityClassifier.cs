...
{
    internal static class PartCapabilityClassifier
    {
            ...
            VesselTopologyNode node)
        {
            ...
            for (int moduleIndex = 0;
                 ...
                 moduleIndex++)
            {
                result.Modules.Add(node.Modules[moduleIndex]);
            }
            ...
            PartCapabilitySnapshot result)
        {
            for (int index = 0;
                 ...
                 index++)
            {
                VesselResourceState state = node.Resources[index];
            ...
            PartCapabilitySnapshot result)
        {
                 ...
                 index++)
            {
                ...
                ResourceDescriptor resource =
                    FindOrCreate(result, requirement.Name);
        ...
        {
            for (int index = 0;
                 index < result.Resources.Count;
                 index++)
            {
                if (string.Equals(
                        result.Resources[index].InternalName,
                        name,
                        StringComparison.OrdinalIgnoreCase))
                {
                    return result.Resources[index];
                }
            }
